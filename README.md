...
Note that the cache is immutable. You can use Clojure's traditional state manangement tools ([the STM](http://clojure.org/reference/refs)) to implement the common case of a stateful cache.

```(require '[clojure.core.cache :as cache])

(def my-cache (atom (cache/lru-cache-factory
                      {:a 100 :b 200}
                      :threshold 512)))

(defn get-value
  [the-cache k]
  (if-let [val (cache/lookup \@the-cache k)]
    (do (swap! the-cache cache/hit k)
        val)
    (let [new-val (rand-int 10)]
      (get (swap! the-cache cache/miss k new-val) k))))

(get-value my-cache :a)
; => 100
(get-value my-cache :b)
; => 200
(get-value my-cache :c)
; => 8
(get-value my-cache :c)
; => 8
```
